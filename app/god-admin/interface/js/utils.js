	...
	{	
		...
		$('form.model-elements-form').ajaxSubmit(
		{
			...
			success: function(data)
			{
				...
				if(data && data.error)
				{
					var errors_message = "<p>" + data.error + "</p>";
				...
				{
					var images = data.uploaded_images.split("<*//*>");
					
					for(var i = 0; i < images.length; i ++)
					{
						var files = images[i].split("/>***</");
						addUploadedMultiImage(element_area_id, {image: files[0], small_image: files[1]});
					}
