...
{	
	...
	if(!in_array($extention, $allowed_extentions)) //Checks extension of image file
...
if(!$error && $type && is_uploaded_file($_FILES['any_file']['tmp_name']))
{
	$folder = dirname($path);
	...
	move_uploaded_file($_FILES['any_file']['tmp_name'], $path); //Copy the file
