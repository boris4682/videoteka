...
class Login
{
	...
	public function rememberUser($id)
	{
		...
		$code = $id.$user['login'].$user['password'].$_SERVER['HTTP_USER_AGENT'];
		$code = password_hash($code, PASSWORD_DEFAULT, array("cost" => 12));
	...
	public function autoLogin($id, $code)
	{
		...
		$check_code = $id.$user['login'].$user['password'].$_SERVER['HTTP_USER_AGENT'];

		if(password_verify($check_code, $code))
