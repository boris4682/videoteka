...
class Form
{
	...
	public function validate()
	{
		...
		if($this -> csrf_token && !$this -> checkTokenCSRF())
	...
	static public function createAjaxTokenCSRF()
	{
		...
		else
		{
			...
			setcookie("key_token", $key, $time, $registry -> getSetting("MainPath"), null, null, $http_only);
		}
				
		return md5($registry -> getSetting("SecretCode").Debug :: browser().$key);
	}
	
	public function useAjaxTokenCSRF()
	{
		$this -> csrf_token = $this -> createAjaxTokenCSRF();
	...
	{
		$csrf_token = self :: createAjaxTokenCSRF();
		
		return "<input type=\"hidden\" name=\"csrf_individual_token\" value=\"".$csrf_token."\" />\n";
	}
