...
class Filemanager
{
	...
	public function displayImage($file)
	{
		...
		if(file_exists($file) && @getimagesize($file))
		{
			...
			$tmp_file = $this -> registry -> getSetting("FilesPath")."tmp/tmp-filemanager-".Service :: randomString(20);
	...
	public function uploadFile($file)
	{
		...
		else if(preg_match("/[^\w-\.]/", basename($new_file)))
	...
	public function cleanTmpFiles()
	{
		...
		while(false !== ($file = readdir($descriptor)))
			if(preg_match("/^tmp-filemanager-\w{20}/", $file))
