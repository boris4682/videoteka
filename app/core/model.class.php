...
{	
	...
	{
		return (get_parent_class($this) == "Model_Simple");
	}
	...
	public function prepareElementValue($object)
	{
		...
		else if($type == 'password' && $object -> getValue())
		{
			if($this -> registry -> getInitialVersion() < 2.2)
				...
				$object -> setValue(trim($object -> getValue()));
		}
			...
			foreach($this -> display_params['table_fields'] as $name)
				if($name != "checkbox" && $name != "actions") //If it's field from db
				{
					...
					if($name == "name" && $this -> getModelClass() == "log" && $this -> registry -> checkModel($row["module"]))
					{
						...
						$model_object = new $model_object();
						$simple_model = (get_parent_class($model_object) == "Model_Simple");
	...
	{
		$images_data = $errors_text = $wrong_files = $uploaded_images = array();
		$old_data = isset($_POST[$name]) ? $_POST[$name] : "";
		...
		return array("input_value" => $old_data,
					 "uploaded_images" => implode("<*//*>", $uploaded_images),
	...
	public function runSorter($fields, $default_field = false, $default_order = false)
	{
		$fields_data = array();
