...
class Pager
{
	...
	public function display($path, $smart)
	{
		...
		foreach ($interval as $value) //Adds pages links one by one
		{
			...
			$html .= "<li class=\"page-item " . $css_class . "\"><a href=\"" . $this->registry->getSetting('SitePath') . $path;
			...
			// if ($css_class)
			// 	$html .= " class=\"" . $css_class . " page-link\"";
			// else 
		...
		if ($current_left > 5 && $this->intervals > 10) //If we need to add the very first page
			$html = "<li class=\"page-item\"><a class=\"very-first page-link\" href=\"" . $this->registry->getSetting('SitePath') . $path . str_replace("number", 1, $pattern) . ">...</a></li>\n" . $html;

		if ($current_right > 5 && $this->intervals > 10) //Id we add very last page
			$html .= "<li class=\"page-item\"><a class=\"very-last page-link\" href=\"" . $this->registry->getSetting('SitePath') . $path . str_replace("number", $this->intervals, $pattern) . ">...</a></li>\n";
	...
	public function displayPrevLink($caption, $path)
	{
		if ($this->checkPrevNext("prev") && $caption) {
			...
			return "<li class=\"page-item\"><a class=\"pager-prev page-link\" href=\"" . $path . "\">" . $caption . "</a></li>\n";
		}
	}
	...
	public function displayNextLink($caption, $path)
	{
		if ($this->checkPrevNext("next") && $caption) {
			$path .= (strpos($path, "?") === false) ? "?" : "&";
			$path .= "page=" . ($this->page + 1);
			return "<li class=\"page-item\"><a class=\"pager-next page-link\" href=\"" . $path . "\">" . $caption . "</a></li>\n";
		}
	}
