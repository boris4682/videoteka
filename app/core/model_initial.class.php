...
abstract class Model_Initial
{
	...
	public function composeSQLQuery()
	{
		...
		$params = $this -> processOneParameterM2M($params);		
	...
	public function processOneParameterM2M($params)
	{
		foreach($params as $key => $value)
			if(strpos($key, "->m2m") && $value)
			{
				$name = str_replace("->m2m", "", $key);
				
				if(array_key_exists("table->", $params))
				{
					$model_object = new $params["table->"]();
					$element = $model_object -> getElement($name);
				}
				else
					$element = $this -> getElement($name);
				
				$table = $element -> getProperty("linking_table");
				$opposite_id = $element -> getOppositeId();
				
				$sql = "SELECT `".$opposite_id."` FROM `".$table."` WHERE `".$name."_id`='".$value."'";
				$ids = $this -> db -> getColumn($sql);
	...
	static public function processSQLConditions($params)
	{
		...
		foreach($params as $key => $value)
		{
			...
			if(in_array($key, $special_keys) || strpos($key, "->m2m"))
				continue;			
			...
			if(strpos($key, "->like") || strpos($key, "->not-like"))
			{
				...
				if($registry -> getSetting("DbEngine") == "mysql")
	...
	static public function processImageArguments($arguments)
	{
		$params = array();
